...
HOSTNAMES=("node-1.intel.r7g.org" "node-2.intel.r7g.org" "node-3.intel.r7g.org" "node-4.intel.r7g.org" "node-5.intel.r7g.org" "node-6.intel.r7g.org" "admin" "kafkaadmintls")
...
openssl req -new -x509 -keyout $TRUSTSTORE_WORKING_DIRECTORY/ca-key \
  ...
  -subj "/C=$COUNTRY/ST=$STATE/L=$LOCATION/O=$OU/CN=CA"
...
keytool -keystore $TRUSTSTORE_WORKING_DIRECTORY/$DEFAULT_TRUSTSTORE_FILENAME \
  ...
  -noprompt -dname "C=$COUNTRY, ST=$STATE, L=$LOCATION, O=$OU, CN=CA" -keypass $PASS -storepass $PASS -storetype PKCS12
...
for CN in "${HOSTNAMES[@]}"; do
  echo
  echo "Now, a keystore will be generated for $CN. Each broker and logical client needs its own"
  echo "keystore. This script will create only one keystore per hostname."
  echo

  keytool -keystore $KEYSTORE_WORKING_DIRECTORY/$KEYSTORE_FILENAME \
    -alias "$CN" -validity $VALIDITY_IN_DAYS -genkey -keyalg RSA \
    -noprompt -dname "C=$COUNTRY, ST=$STATE, L=$LOCATION, O=$OU, CN=$CN" -keypass $PASS -storepass $PASS

  echo
  echo "'$KEYSTORE_WORKING_DIRECTORY/$KEYSTORE_FILENAME' for $CN now contains a key pair and a"
  ...
  echo
  echo "Now a certificate signing request will be made to the keystore for $CN."
  echo

  keytool -keystore $KEYSTORE_WORKING_DIRECTORY/$KEYSTORE_FILENAME -alias "$CN" \
  ...
  # Check if the alias already exists before importing
  if keytool -list -keystore $KEYSTORE_WORKING_DIRECTORY/$KEYSTORE_FILENAME -alias CARoot -storepass $PASS &> /dev/null; then
    echo "Certificate with alias 'CARoot' already exists in the keystore."
  else
  ...
  keytool -keystore $KEYSTORE_WORKING_DIRECTORY/$KEYSTORE_FILENAME -alias "$CN" -import \
    -file $KEYSTORE_SIGNED_CERT -keypass $PASS -storepass $PASS

  echo
  echo "Keystore for $CN is now complete!"
done
