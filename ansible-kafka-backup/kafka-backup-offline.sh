...
{
    # online actions to minimize downtime
...
function cluster_reinstall()
{
    ...
    # wipe data (without containers running)
    cluster_containers_remove
    cluster_data_format

    # apply ACL, containers has to run, and they will generate errors as running without ACLs.
    cluster_containers_run
    cluster_acls_apply

    # start containers from scratch, to remove error generated by cluster during run without ACL
    cluster_containers_remove
    cluster_containers_run
}
