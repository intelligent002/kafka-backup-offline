...
function cluster_reinstall()
{
    ...
    # apply ACL (containers has to run, and they will generate errors)
    cluster_containers_run
    cluster_acls_apply

    # start containers from scratch, to remove error logs generated by cluster during run without ACL
    cluster_containers_remove
    cluster_containers_run
}
